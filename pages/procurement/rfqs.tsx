...
import {
  Search, FileText, Clock, DollarSign, Users, ArrowRight,
  ChevronLeft, ChevronRight, Loader2, Calendar, Tag
} from 'lucide-react';
...
export default function RfqsListPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ type: 'rfqs', page: String(page), limit: String(limit) });
  ...
  return (
    <PortalLayout title="Daftar RFQ" description="Lihat semua Request for Quotation yang sedang dibuka">
      <div className="bg-gradient-to-br from-emerald-600 to-teal-700 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 relative">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Request for Quotation</h1>
          </div>
          <p className="text-white/70 ml-[52px]">Permintaan penawaran harga dari perusahaan</p>
            ...
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input className="w-full pl-11 pr-4 py-3 bg-white rounded-xl text-sm shadow-lg outline-none focus:ring-2 focus:ring-white/20"
                  placeholder="Cari RFQ..." value={search}
                  onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchData()} />
              </div>
              <button onClick={() => { setPage(1); fetchData(); }}
                className="px-6 py-3 bg-white text-emerald-700 font-bold rounded-xl shadow-lg hover:bg-emerald-50 transition-all text-sm">
                Cari
              </button>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-gray-500">{total} RFQ ditemukan</p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-20 gap-4">
            ...
            <p className="text-sm text-gray-500">Memuat RFQ...</p>
          </div>
        ) : data.length === 0 ? (
          <div className="text-center py-20">
            <div className="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center mx-auto mb-4"><FileText className="w-8 h-8 text-gray-300" /></div>
            <h3 className="font-semibold text-gray-600">Belum ada RFQ terbuka</h3>
            <p className="text-sm text-gray-400 mt-1">RFQ baru akan muncul di sini saat dipublikasikan</p>
          </div>
        ) : (
          <>
            <div className="space-y-4">
              {data.map((item: any) => {
                const dl = daysUntil(item.closing_date);
                const isUrgent = dl !== null && dl >= 0 && dl <= 7;
                const isClosed = dl !== null && dl < 0;
                return (
                  <Link key={item.id} href={`/procurement/${item.id}?type=rfq`}
                    ...
                    <div className="p-5 sm:p-6 flex flex-col sm:flex-row gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 ring-1 ring-emerald-600/20">Dibuka</span>
                        ...
                        <h3 className="text-lg font-bold text-gray-900 group-hover:text-emerald-600 transition-colors truncate">{item.title}</h3>
                        ...
                        <div className="flex flex-wrap gap-4 mt-3 text-xs text-gray-500">
                          <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> Deadline: <strong className={isClosed ? 'text-red-500' : ''}>{fD(item.closing_date)}</strong></span>
                          <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> {item.bid_count || 0} respons</span>
                        </div>
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-10">
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => { const p = page <= 3 ? i + 1 : page + i - 2; if (p < 1 || p > totalPages) return null; return (<button key={p} onClick={() => setPage(p)} className={`w-10 h-10 rounded-xl text-sm font-medium transition-all ${p === page ? 'bg-emerald-600 text-white shadow-lg' : 'border bg-white text-gray-600 hover:bg-gray-50'}`}>{p}</button>); })}
