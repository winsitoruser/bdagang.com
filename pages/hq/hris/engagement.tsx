...
export default function EngagementPage() {
  ...
  const tabs: { key: TabKey; label: string; icon: any; count?: number }[] = [
    { key: 'surveys', label: 'Survey & Pulse', icon: BarChart3, count: overview.totalSurveys },
    { key: 'recognitions', label: 'Recognition & Reward', icon: Award, count: overview.totalRecognitions },
    { key: 'announcements', label: 'Pengumuman', icon: Bell, count: overview.publishedAnnouncements },
  ];

  return (
    <HQLayout title="Employee Engagement & Culture">
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Employee Engagement & Culture</h1>
        <p className="text-gray-500 mt-1">Survey, recognition, dan komunikasi internal</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl p-4 text-white">
          ...
          <p className="text-xs opacity-80">Total Survey</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-amber-600 rounded-xl p-4 text-white">
          ...
          <p className="text-xs opacity-80">Recognition</p>
        </div>
      ...
      {!loading && tab === 'surveys' && !surveyDetail && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">Survey & Pulse Check</h2>
              ...
              <div key={s.id} className="bg-white border rounded-xl p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {s.is_anonymous && <span className="text-xs text-gray-400">🔒 Anonymous</span>}
                    </div>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t">
                  ...
                  <div className="flex gap-1">
                    ...
                    {s.status === 'draft' && (
                      <button onClick={async () => { await api('publish-survey', 'POST', { id: s.id }); showToast('Survey dipublikasikan'); loadData(); }} className="text-xs px-2 py-1 text-green-600 hover:bg-green-50 rounded">Publish</button>
                    )}
                    {s.status === 'active' && (
      ...
      {!loading && tab === 'recognitions' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">e-Recognition & Rewards</h2>
            <button onClick={() => openAdd('recognition')} className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg text-sm hover:bg-orange-700">
              <Award className="w-4 h-4" /> Beri Recognition
            </button>
          </div>
          <div className="space-y-3">
            ...
            {recognitions.length === 0 && <p className="text-center text-gray-400 py-8">Belum ada recognition. Mulai apresiasi rekan kerja!</p>}
          </div>
        </div>
      )}
              ...
              <div key={a.id} className={`bg-white border rounded-xl p-4 ${a.is_pinned ? 'border-blue-300 bg-blue-50/30' : ''}`}>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    {a.status === 'draft' && (
                      <button onClick={async () => { await api('publish-announcement', 'POST', { id: a.id }); showToast('Pengumuman diterbitkan'); loadData(); }} className="text-xs px-2 py-1 text-green-600 hover:bg-green-50 rounded">Publish</button>
                    )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-5 border-b">
              <h3 className="text-lg font-semibold">
                {editingItem ? 'Edit' : 'Buat'} {modalType === 'survey' ? 'Survey' : modalType === 'recognition' ? 'Recognition' : 'Pengumuman'}
              </h3>
              ...
              {modalType === 'survey' && (<>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-end gap-4 pb-2">
                    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={surveyForm.isAnonymous} onChange={e => setSurveyForm({ ...surveyForm, isAnonymous: e.target.checked })} /> Anonymous</label>
              ...
              {modalType === 'recognition' && (<>
                ...
                <div><label className="text-sm font-medium text-gray-700">Tipe</label>
                  <select value={recForm.recognitionType} onChange={e => setRecForm({ ...recForm, recognitionType: e.target.value })} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm">
                    <option value="kudos">👏 Kudos</option><option value="achievement">🏆 Achievement</option><option value="milestone">🎯 Milestone</option><option value="teamwork">🤝 Teamwork</option><option value="innovation">💡 Innovation</option><option value="leadership">👑 Leadership</option><option value="service">⭐ Service Excellence</option>
                  </select>
                </div>
