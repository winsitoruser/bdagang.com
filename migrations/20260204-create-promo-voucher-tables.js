...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const hasPromos = tableNames.some(
      (n) => String(n).toLowerCase() === 'promos'
    );
    const hasVouchers = tableNames.some(
      (n) => String(n).toLowerCase() === 'vouchers'
    );
    ...
    const p = await queryInterface.describeTable('promos');
    const v = await queryInterface.describeTable('vouchers');
    if (p.code) {
      await seq.query(
        `CREATE UNIQUE INDEX IF NOT EXISTS promos_code_unique ON promos (code);`
      );
    }
    if (p.status) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS promos_status_index ON promos (status);`
      );
    }
    if (p.startDate && p.endDate) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS promos_dates_index ON promos ("startDate", "endDate");`
      );
    } else if (p.start_date && p.end_date) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS promos_dates_index ON promos (start_date, end_date);`
      );
    }
    if (v.code) {
      await seq.query(
    ...
    if (v.category) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS vouchers_category_index ON vouchers (category);`
      );
    }
    if (v.status) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS vouchers_status_index ON vouchers (status);`
      );
    }
    if (v.validFrom && v.validUntil) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS vouchers_dates_index ON vouchers ("validFrom", "validUntil");`
      );
    } else if (v.valid_from && v.valid_until) {
      await seq.query(
        `CREATE INDEX IF NOT EXISTS vouchers_dates_index ON vouchers (valid_from, valid_until);`
      );
    }

    const pCols = await queryInterface.describeTable('promos').catch(() => ({}));
    const seedCamelPromos =
      pCols.description !== undefined &&
      pCols.minPurchase !== undefined &&
      pCols.startDate !== undefined;
    const [[{ pc }]] = await seq.query(
      'SELECT COUNT(*)::int AS c FROM promos'
    );

    if (seedCamelPromos && Number(pc) === 0) {
    ...
    const vCols = await queryInterface.describeTable('vouchers').catch(() => ({}));
    const seedCamelVouchers =
      vCols.validFrom !== undefined &&
      vCols.validUntil !== undefined &&
      vCols.applicableFor !== undefined;
    const [[{ vc }]] = await seq.query(
      'SELECT COUNT(*)::int AS c FROM vouchers'
    );

    if (seedCamelVouchers && Number(vc) === 0) {
